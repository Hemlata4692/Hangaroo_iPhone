...
- (id)initWithFrame:(CGRect)frame backGroudImage:(NSString *)backImageName headerImageURL:(NSString *)headerImageURL title:(NSString *)title subTitle:(NSString *)subTitle buttonTitle:(NSString *)buttonTitle
{
    ...
    if (self) {
        ...
        _settings = [[UIButton alloc] initWithFrame:CGRectMake(frame.size.width-40, 60, 40, 20)];
        _settings.backgroundColor=[UIColor blackColor];
        _settings.titleLabel.font = [UIFont systemFontOfSize:14];
        _settings.titleLabel.text = buttonTitle;
        _settings.titleLabel.textColor=[UIColor yellowColor];
...
-(void)updateSubViewsWithScrollOffset:(CGPoint)newOffset
{
    ...
    CGAffineTransform t = CGAffineTransformMakeTranslation(0,(titleDestinateOffset-0.20*self.frame.size.height)*(1-alpha));
